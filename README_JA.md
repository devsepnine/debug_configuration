...
- セッションごとの stdin 入力バー (トグルボタンまたは Cmd+I): プロンプト応答や
  ...
  (1809 未満) のみアプリがローカルエコー。送信した行はセッションごとの履歴に
  保存 (↑/↓ で呼び出し)、^C ボタンまたは入力バーフォーカス中の Ctrl+C で
  割り込みを送信、^D / Ctrl+D で EOF を送信 (REPL・stdin リーダーを終了)
- リアルタイム出力表示 (ANSI カラー対応; OSC/DCS 制御シーケンスは除去)
- セッションの再実行、停止、削除、ワークスペースから非表示
- 終了セッションのステータスバッジ (成功 / 失敗時の終了コード / 実行時間)
- 終了セッションはわずかに暗く表示され、実行中のペインと一目で区別
- ウィンドウが非アクティブな状態で実行完了したときのデスクトップ通知
- 一括操作: すべて停止、すべて再実行、失敗のみ再実行
- ワークスペース別の開閉状態を持つ永続セッションリスト
- 実行中のセッションはリスト上部に留まり、終了セッションは件数付きの
  区切り線の下へ移動
- ワークスペースタブシステム
  - 最低 1 つのワークスペースタブを保持
...
- ペインベースのワークスペースレイアウト
  ...
  - ペインのリサイズ
  - 複数ペイン時の最大化/復元
  - 幅の狭いペインではコンパクトなオーバーフロー (⋯) メニューを表示、ペインごとに独立して開閉
...
- **Windows の特記事項**: ConPTY はレンダリング済みの VT ストリームを渡し、
  ...
  で近似されます (バックスペース消去、キャリッジリターンで行書き換え)。実行中の
  セッションにはペインのリサイズが伝播されません (ConPTY はライブリサイズに全画面
  再描画で応答し、スクロールバックに出力が重複するため) — 新しいサイズは次回の
  実行/再実行から適用されます。新しいセッションは直近に観測されたペインサイズで
  起動し (観測前は 120×40)、この推定が外れた場合も次回の実行/再実行まで維持
  されます。Stop は
  ...
  エコー、割り込み (^C) 伝達不可、セッションにバナー表示。予期しない PowerShell
  プロンプトの待機 (ハングのように見える — Stop で終了) の注意はこの
  フォールバックのみに該当します。
- **stdin 入力バーの Ctrl+C は割り込み**: セッションの stdin 入力バーが
  フォーカス中の Ctrl+C は常にプロセスへ割り込みを送信します。Windows/Linux で
  入力バーに選択範囲がある場合はコピーも併せて行われます。出力領域のコピーと
  macOS の Cmd+C コピーには影響しません。
- **Ctrl+D は EOF**: 実際の端末と同様、行頭で作用します — stdin 入力バーの
  未送信ドラフトは先に送信されません (Enter で送信してから Ctrl+D)。Windows は
  内部的にコンソールの EOF シーケンス (Ctrl+Z+Enter) を送り、旧式 pipe
  フォールバックは stdin ハンドルを閉じて EOF にします。
