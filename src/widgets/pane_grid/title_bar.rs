//! `TitleBar` widget for `pane_grid` - MODIFIED for title content drag support
//!
//! This is a vendored version with modified `is_over_pick_area` logic.
//! The upstream iced 0.14 implementation excludes title content from the drag
//! pick area, which prevents dragging when there's text in the `TitleBar`.
//!
//! MODIFICATION: Title content area is now included in the pick area,
//! allowing drag to work even when clicking on text.
...
///
/// [`Pane`]: super::Pane
pub struct TitleBar<'a, Message, Theme = iced::Theme, Renderer = iced::Renderer>
where
    ...
    Renderer: advanced::Renderer,
{
    ...
    /// [`TitleBar`] or not.
    ///
    /// **MODIFIED**: The whole [`TitleBar`] is a pick area, except its controls.
    /// Unlike upstream iced 0.14, title content IS included in the pick area,
    /// allowing drag to work when clicking on text.
    pub fn is_over_pick_area(&self, layout: layout::Layout<'_>, cursor_position: Point) -> bool {
        if layout.bounds().contains(cursor_position) {
            let mut children = layout.children();
            let padded = children.next().unwrap();
            let mut children = padded.children();
            let title_layout = children.next().unwrap(); // Read for compact-controls layout sizing only
            ...
            if let Some(controls) = self.controls.as_ref() {
                    ...
                    > padded.bounds().width
                {
                    if controls.compact.is_some() {
                        ...
                        // MODIFIED: Only exclude compact controls, NOT title content
                        !compact_layout.bounds().contains(cursor_position)
                    } else {
                        // MODIFIED: Only exclude controls, NOT title content
                ...
                // MODIFIED: No controls, entire TitleBar (including title content) is pick area
                true
            }
        } else {
            false
