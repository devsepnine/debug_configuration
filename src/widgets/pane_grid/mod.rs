...
//!
//! This is a vendored version of iced's `pane_grid` with modified `TitleBar` drag behavior.
//! The modification allows dragging from anywhere on the `TitleBar`, including over text content.
...
// Controls는 현재 사용하지 않지만 향후 TitleBar에 버튼 추가 시 필요
...
use iced::advanced::overlay::{self, Group};
    ...
    Renderer: advanced::Renderer,
{
    ...
    fn active_on_drag(&self) -> Option<&dyn Fn(DragEvent) -> Message> {
        if self.drag_enabled() {
            self.on_drag.as_deref()
        } else {
            None
        }
    }
    ...
    fn handle_press_event(
        ...
        cursor: mouse::Cursor,
        shell: &mut advanced::Shell<'_, Message>,
        on_drag: Option<&dyn Fn(DragEvent) -> Message>,
    ) {
        let bounds = layout.bounds();
        ...
        if let Some(cursor_position) = cursor.position_over(bounds) {
            ...
            if !self.try_start_resize(action, layout, cursor_position) {
                click_pane(
                    action,
                    layout,
                    cursor_position,
    ...
    fn try_start_resize(
        ...
        let Some((leeway, _)) = &self.on_resize else {
            return false;
        };
        ...
        let splits =
            self.internal
                .layout()
                .split_regions(self.spacing, self.min_size, bounds.size());

        let clicked_split = hovered_split(splits.iter(), self.spacing + leeway, relative_cursor);

        if let Some((split, axis, _)) = clicked_split
            && action.picked_pane().is_none()
        {
    ...
    fn handle_release_event(
        ...
        {
            let event = self.drag_event_for_release(layout, pane, origin, cursor_position);
    ...
    fn drag_event_for_release(
        ...
        if cursor_position.distance(origin) <= DRAG_DEADBAND_DISTANCE {
            return DragEvent::Canceled { pane };
        }

        if let Some(edge) = in_edge(layout, cursor_position) {
            return DragEvent::Dropped {
                pane,
        ...
        let dropped_region = self
            ...
            .find_map(|(target, layout)| {
                layout_region(layout, cursor_position).map(|region| (target, region))
            });

        match dropped_region {
            Some(((target, _), region)) if pane != target => DragEvent::Dropped {
                pane,
    ...
    fn handle_cursor_moved_event(
        ...
        if let Some((_, on_resize)) = &self.on_resize {
            if let Some((split, _)) = action.picked_split() {
                self.publish_resize(layout, cursor, split, on_resize, shell);
    ...
    fn publish_resize(
        ...
        cursor: mouse::Cursor,
        split: Split,
        on_resize: &dyn Fn(ResizeEvent) -> Message,
        shell: &mut advanced::Shell<'_, Message>,
    ) {
        let bounds = layout.bounds();
        let splits =
            ...
            .or_else(|| {
                self.panes
                    ...
                    .find_map(|((_pane, content), layout)| {
                        content.grid_interaction(layout, cursor, drag_enabled)
                    })
            })
    ...
    fn picked_split_highlight(
        &self,
        action: &state::Action,
        layout: layout::Layout<'_>,
        cursor: mouse::Cursor,
    ) -> Option<(Axis, Rectangle, bool)> {
        let node = self.internal.layout();
        let resize_leeway = self.on_resize.as_ref().map(|(leeway, _)| *leeway);
    ...
    fn render_panes(
        ...
        cursor: mouse::Cursor,
        viewport: &Rectangle,
        picked_pane: Option<(Pane, Point)>,
        pane_cursor: mouse::Cursor,
        style: &Style,
    ) {
        ...
        {
            match picked_pane {
                Some((dragging, _origin)) if id == dragging => {}
    ...
    fn draw_picked_pane(
        ...
        let Some((picked_id, origin)) = picked_pane else {
            return;
        };
    ...
    fn draw_split_highlight(
        renderer: &mut Renderer,
        picked_pane: Option<(Pane, Point)>,
        picked_split: Option<(Axis, Rectangle, bool)>,
        style: &Style,
    ) {
    ...
    Renderer: advanced::Renderer,
{
    ...
    fn update(
        ...
        let picked_pane = {
            ...
            action.picked_pane().map(|(pane, _)| pane)
        };
        self.update_pane_contents(
            tree,
            event,
            layout,
            cursor,
        ...
        match event {
            ...
            | Event::Touch(iced::touch::Event::FingerPressed { .. }) => {
                let on_drag = self.active_on_drag();
                let Memory { action, .. } = tree.state.downcast_mut();
                self.handle_press_event(action, layout, cursor, shell, on_drag);
            }
            ...
            | Event::Touch(
                ...
                let on_drag = self.active_on_drag();
                let Memory { action, .. } = tree.state.downcast_mut();
                self.handle_release_event(action, layout, cursor, shell, on_drag);
            }
            Event::Mouse(mouse::Event::CursorMoved { .. })
            | Event::Touch(iced::touch::Event::FingerMoved { .. }) => {
                let Memory { action, .. } = tree.state.downcast_ref();
                self.handle_cursor_moved_event(action, layout, cursor, shell);
            }
            _ => {}
        }

        let Memory { action, .. } = tree.state.downcast_ref();
        self.sync_mouse_interaction(
            action,
            event,
            layout,
            cursor,
            shell,
            self.active_on_drag().is_some(),
        );
    }
    ...
    fn draw(
        ...
        let picked_pane = action.picked_pane().filter(|(_, origin)| {
            cursor
                .position()
                .is_some_and(|position| position.distance(*origin) > DRAG_DEADBAND_DISTANCE)
        });
        let picked_split = self.picked_split_highlight(action, layout, cursor);
        ...
        Self::draw_split_highlight(renderer, picked_pane, picked_split, &style);
    }
...
fn layout_region(layout: layout::Layout<'_>, cursor_position: Point) -> Option<Region> {
    let bounds = layout.bounds();
...
fn click_pane<'a, Message, T>(
    ...
    on_click: Option<&(dyn Fn(Pane) -> Message + 'a)>,
    on_drag: Option<&(dyn Fn(DragEvent) -> Message + 'a)>,
) where
    T: Draggable,
{
...
fn in_edge(layout: layout::Layout<'_>, cursor: Point) -> Option<Edge> {
    let bounds = layout.bounds();
...
fn edge_bounds(layout: layout::Layout<'_>, edge: Edge) -> Rectangle {
    let bounds = layout.bounds();
...
fn layout_region_bounds(layout: layout::Layout<'_>, region: Region) -> Rectangle {
    let bounds = layout.bounds();
